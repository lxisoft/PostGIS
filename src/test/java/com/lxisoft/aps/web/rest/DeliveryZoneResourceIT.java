...
class DeliveryZoneResourceIT {
    ...
    private static final String DEFAULT_BOUNDARY_STRING = "AAAAAAAAAA";
    private static final String UPDATED_BOUNDARY_STRING = "BBBBBBBBBB";
    ...
    public static DeliveryZone createEntity() {
        return new DeliveryZone().name(DEFAULT_NAME).active(DEFAULT_ACTIVE).boundaryString(DEFAULT_BOUNDARY_STRING);
    }
    ...
    public static DeliveryZone createUpdatedEntity() {
        return new DeliveryZone().name(UPDATED_NAME).active(UPDATED_ACTIVE).boundaryString(UPDATED_BOUNDARY_STRING);
    }
    ...
    @Test
    void getAllDeliveryZonesAsStream() {
        // Initialize the database
        deliveryZoneRepository.save(deliveryZone).block();

        List<DeliveryZone> deliveryZoneList = webTestClient
            .get()
            .uri(ENTITY_API_URL)
            ...
            .returnResult(DeliveryZoneDTO.class)
            .getResponseBody()
            .map(deliveryZoneMapper::toEntity)
            .filter(deliveryZone::equals)
            .collectList()
            .block(Duration.ofSeconds(5));

        assertThat(deliveryZoneList).isNotNull();
        assertThat(deliveryZoneList).hasSize(1);
        DeliveryZone testDeliveryZone = deliveryZoneList.get(0);

        // Test fails because reactive api returns an empty object instead of null
        // assertDeliveryZoneAllPropertiesEquals(deliveryZone, testDeliveryZone);
        assertDeliveryZoneUpdatableFieldsEquals(deliveryZone, testDeliveryZone);
    }
    ...
    void getAllDeliveryZones() {
        ...
        webTestClient
            ...
            .jsonPath("$.[*].boundaryString")
            .value(hasItem(DEFAULT_BOUNDARY_STRING));
    }
    ...
    void getDeliveryZone() {
        ...
        webTestClient
            ...
            .jsonPath("$.boundaryString")
            .value(is(DEFAULT_BOUNDARY_STRING));
    }
    ...
    void putExistingDeliveryZone() throws Exception {
        ...
        updatedDeliveryZone.name(UPDATED_NAME).active(UPDATED_ACTIVE).boundaryString(UPDATED_BOUNDARY_STRING);
    ...
    void partialUpdateDeliveryZoneWithPatch() throws Exception {
        ...
        partialUpdatedDeliveryZone.name(UPDATED_NAME);
    ...
    void fullUpdateDeliveryZoneWithPatch() throws Exception {
        ...
        partialUpdatedDeliveryZone.name(UPDATED_NAME).active(UPDATED_ACTIVE).boundaryString(UPDATED_BOUNDARY_STRING);
