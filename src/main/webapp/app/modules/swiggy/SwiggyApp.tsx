...
const FOOD_ITEMS = [
  { name: 'Special Chicken Biryani', price: '₹180' },
  { name: 'Paneer Butter Masala & Roti', price: '₹150' },
  { name: 'Cheesy Loaded Pizza', price: '₹220' },
  { name: 'Special South Indian Meals', price: '₹120' },
];

export const SwiggyApp: React.FC = () => {
  ...
  const [customerName, setCustomerName] = useState('Arjun Kumar');
  ...
  const [selectedFoodItem, setSelectedFoodItem] = useState(FOOD_ITEMS[0].name);
  ...
  const loadAllData = async () => {
    try {
      // Reset all delivery partners to AVAILABLE so the demo always starts fresh
      await fetch('/api/swiggy/reset', { method: 'POST' });

      const res = await fetch('/api/swiggy/all-data');
      if (res.ok) {
      ...
      console.error('Failed to load Swiggy base data', err);
    } finally {
      setLoading(false);
    }
  };

  // Evaluate zone containment using PostGIS ST_Contains via API
  const handleMapClick = async (lat: number, lng: number) => {
    if (orderStep !== 'welcome' && orderStep !== 'ready_to_order' && orderStep !== 'invalid_zone') {
      return;
    }
    ...
    try {
      const res = await fetch(`/api/swiggy/zones/validate?lat=${lat}&lng=${lng}`);
      if (res.ok) {
        const zones: IDeliveryZone[] = await res.json();
        if (zones.length > 0) {
          const matchedZone = zones[0];
          setActiveZone(matchedZone);

          // Get nearby restaurants using PostGIS ST_DWithin (3km radius)
          const restsRes = await fetch(`/api/swiggy/restaurants/nearby?lat=${lat}&lng=${lng}&radiusKm=3`);
          if (restsRes.ok) {
            const rests: IRestaurant[] = await restsRes.json();
  ...
  return (
    <div className="swiggy-app">
            ...
            {(orderStep === 'ready_to_order' || orderStep === 'submitting_order') && (
              <div className="swiggy-card-group">
                ...
                <div className="swiggy-card swiggy-glass">
                    ...
                    <div className="swiggy-restaurants-list">
                      {nearbyRestaurants.map(r => (
                        ...
                        >
                          ...
                          {r.distance !== undefined && <span className="rest-distance">{(r.distance / 1000).toFixed(2)} km</span>}
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {selectedRestaurant && (
                    <div className="swiggy-order-selection">
      ...
      <div className="swiggy-map-container">
        {!loading && (
          <MapContainer center={[10.7716, 76.3762]} zoom={13} className="swiggy-map" style={{ height: '100%', width: '100%' }}>
            ...
            {orderStep === 'on_the_way' && polylineCoords.length > 1 && (
              <>
                ...
                <LeafletCircle center={polylineCoords[0]} radius={500} color="#EF4444" fillColor="#EF4444" fillOpacity={0.5} weight={1} />
              </>
            )}
          </MapContainer>
        )}
