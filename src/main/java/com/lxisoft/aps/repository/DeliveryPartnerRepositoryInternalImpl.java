...
class DeliveryPartnerRepositoryInternalImpl
    ...
    public Mono<DeliveryPartner> findNearestAvailable(Double lat, Double lng) {
        // Use ::geometry cast for the <-> KNN operator — PostGIS KNN index scan requires geometry, not geography.
        // ST_DWithin with geography is used for accurate metre-based filtering, but for ordering/LIMIT 1
        // the geometry <-> operator with the GiST index gives the correct nearest-neighbour result.
        String query =
